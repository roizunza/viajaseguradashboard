...
export default function ChartsContainer() {
  ...
  const CustomTooltip = ({ active, payload, label }) => {
    if (active && payload && payload.length) {
      return (
        <div style={{ 
          backgroundColor: 'rgba(24, 29, 53, 0.95)', 
          ...
          {payload.map((entry, index) => (
            <div key={index} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              <span style={{ color: '#E0E0E0' }}>
                {entry.name}: <span style={{ color: '#fff', fontWeight: 'bold' }}>{Math.abs(entry.value)}</span>
              </span>
            </div>
          ))}
        </div>
  ...
  return (
    <div style={{ display: 'flex', width: '100%', height: '100%', padding: '20px' }}>
      ...
      <div style={{ flex: 65, display: 'flex', flexDirection: 'column', paddingRight: '20px' }}>
        ...
        <div style={{ display: 'flex', flex: 1, gap: '2px' }}> 
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
            <div style={{ flex: 1, minHeight: 0 }}>
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={dataOyamel} layout="vertical" stackOffset="sign" margin={{top:0, right:2, left:0, bottom:0}}>
                  <XAxis type="number" hide />
                  <YAxis type="category" dataKey="name" hide width={10} />
                  <Tooltip content={<CustomTooltip />} cursor={{fill: 'rgba(255,255,255,0.05)'}} />
                  <Bar dataKey="Ascensos" fill={COLORS.rutas.Oyamel} stackId="stack" radius={[0, 3, 3, 0]} barSize={22} />
          ...
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
            <div style={{ flex: 1, minHeight: 0 }}>
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={dataOcotal} layout="vertical" stackOffset="sign" margin={{top:0, right:2, left:2, bottom:0}}>
                  <XAxis type="number" hide />
                  <YAxis type="category" dataKey="name" hide />
                  <Tooltip content={<CustomTooltip />} cursor={{fill: 'rgba(255,255,255,0.05)'}} />
                  <Bar dataKey="Ascensos" fill={COLORS.rutas.Ocotal} stackId="stack" radius={[0, 3, 3, 0]} barSize={22} />
                  <Bar dataKey="Descensos" fill={COLORS.descensos} stackId="stack" radius={[3, 0, 0, 3]} barSize={22} />
                </BarChart>
              </ResponsiveContainer>
            </div>
            <div style={{...subChartTitleStyle, color: COLORS.rutas.Ocotal}}>Ocotal</div>
          </div>

          <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
            <div style={{ flex: 1, minHeight: 0 }}>
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={dataAntigua} layout="vertical" stackOffset="sign" margin={{top:0, right:0, left:2, bottom:0}}>
                  <XAxis type="number" hide />
                  <YAxis type="category" dataKey="name" hide />
                  <Tooltip content={<CustomTooltip />} cursor={{fill: 'rgba(255,255,255,0.05)'}} />
                  <Bar dataKey="Ascensos" fill={COLORS.rutas.Antigua} stackId="stack" radius={[0, 3, 3, 0]} barSize={22} />
                  <Bar dataKey="Descensos" fill={COLORS.descensos} stackId="stack" radius={[3, 0, 0, 3]} barSize={22} />
                </BarChart>
              </ResponsiveContainer>
            </div>
            <div style={{...subChartTitleStyle, color: COLORS.rutas.Antigua}}>Antigua</div>
          </div>
        </div>
      </div>
      ...
      <div style={{ flex: 30, display: 'flex', flexDirection: 'column', paddingLeft: '20px', paddingRight: '40px' }}>
        ...
        <div style={{ flex: 1, minHeight: 0 }}>
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={dataEquip} margin={{top:10, right:0, left:-20, bottom:0}} barCategoryGap="20%">
